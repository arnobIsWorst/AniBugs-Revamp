...
import { getReviewsOfCharacter } from '../../api'
import Rating from '../Stars'

function Characterrates({character}) {
  const Ratings = [
    {
      title:"0 - Poor",
      value:0,
  ...
  useEffect(() => {
    const fetchReviews = async () => {
      try {
        const data = await getReviewsOfCharacter(character?.id)
    ...
    <div className='my-12'>
      ...
      <div className='mt-10 xl:grid flex-colo grid-cols-5 gap-12 bg-dry xs:p-10 py-10 px-2 sm:p-20 rounded'>
        ...
        <div className='xl:col-span-2 w-full flex flex-col gap-8'>
          <h3 className='text-xl text-text font-semibold '>Review "{character?.name}"
          </h3>
          <p className='text-sm leading-7 font-medium text-border'>
            Write a review for this Character.
          </p>
