...
CREATE OR REPLACE PROCEDURE DELETE_ANIME(P_ANIME_ID, IN NUMBER)
...
-- Function to find if a studio is followed by a user
CREATE OR REPLACE FUNCTION IS_FOLLOWED(P_USER_ID IN NUMBER, P_STUDIO_ID IN NUMBER)
RETURN BOOLEAN IS
FLAG NUMBER;
BEGIN
...
CREATE OR REPLACE FUNCTION is_followed(p_user_id_param INTEGER, p_studio_id_param INTEGER)
RETURNS BOOLEAN AS $$
DECLARE
    flag_param INTEGER;
BEGIN
    SELECT COUNT(*) INTO flag_param
    FROM follow
    WHERE user_id = p_user_id_param AND studio_id = p_studio_id_param;
